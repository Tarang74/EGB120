...
\end{equation*}
where the ratio of $\frac{R_2}{R_1}$ must be equal on both legs.
\subsection{Designing Op Amps}
In each
of the following circuits, it is important to choose resistors in the \SI{1}{\kilo\ohm} - \SI{100}{\kilo\ohm}
...
These signals will be described using the cosine function.
\begin{equation*}
    v = V_{\mathrm{rms}} \cos{\left( \omega t + \phi \right)}
\end{equation*}
...
\begin{definition}[Period]
    The period of a signal is its cycle time $T$ in seconds (\si{\second)}.
\end{definition}
...
\subsubsection{Phase}
\begin{definition}
    The phase of a signal is the position of a signal relative to a zero phase signal, 
...
\begin{definition}
    ...
    \end{equation*}
    The units in rms quantities are also subscripted with rms, for example, $\si{\volt}_{\mathrm{rms}}$.
\end{definition}
\subsection{Power}
\begin{definition}
    ...
    \begin{align*}
        p &= vi \\
        p &= 2V_{\mathrm{rms}}I_{\mathrm{rms}}\cos{\left( \omega t + \phi \right)}\cos{\left( \omega t + \phi \right)} \\
