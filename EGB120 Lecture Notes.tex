...
\begin{definition}
    ...
    to charge across a capacitor is its capacitance, measured in Farads (\unit{F}).
    \begin{equation*}
        q = C v
    \end{equation*}
\end{definition}
...
\begin{definition}
    ...
    to the rate of change of current through an inductor is its inductance, measured in Henrys (\unit{H}).
\end{definition}
\begin{definition}[VI Relationship]
    \begin{equation*}
        v = L \odv{i}{t}
...
\begin{definition}
    The angular frequency \(\omega\) is measured in radians per second (\unit{rad.s^{-1}}), and it allows
    us to compute the cosine function in radians.
\end{definition}
\subsubsection{Period and Frequency}
\begin{definition}[Period]
    The period of a signal \(T\) is its cycle time in seconds (\unit{s}).
\end{definition}
\begin{definition}[Frequency]
    The frequency of a signal \(f\) is the number of cycles per second, or
    the inverse of the period. The frequency is measured in Hertz (\unit{Hz}).
\end{definition}
...
\begin{definition}
    The impedance \(\symbf{Z}\) of an element, measured in Ohms (\unit{\Omega}), is its opposition to alternating current.
...
In this plot, the frequency (in \unit{Hz}) is plotted on the horizontal axis using a logarithmic scale.
...
\begin{enumerate}
    \item LM78XX regulates a positive voltage XX\unit{V}.
    \item LM79XX regulates a negative voltage -XX\unit{V}.
\end{enumerate}
